...
export default function Game(props) {
  ...
  useEffect(() => {
    ...
    Object.keys(user).map(item => {
      if (user[item]){          
        loadBuilding(item, gold, prestige, goldRequired, setGoldPerSecond, setDetriment);
      }
    });
  }, [user, prestige]);
  ...
  useEffect(() => {
    if (gametime % 3 === 0){
      ...
      detrimentRoll(detriment, gametime, user, setGoldPerSecond, setActiveTime, setGold, setUser);
    }
    shutOffEffect(detriment,  gametime, activeTime, setGoldPerSecond, setDetriment);
  }, [gametime]);
    ...
    <div>
      <div className={style.overhead}>
        <UserControls handleMineClick={mineGold} 
          ...
          user={user} setUser={setUser} gold={gold} auth={props.auth} setLoadUser={setLoadUser} prestige={prestige} gametime={gametime} numClicks={numClicks}/>
        <Prestige 
          user={user} 
          prestige={prestige} 
          ...
          setUser={setUser} 
          setGold={setGold} 
          setGoldPerSecond={setGoldPerSecond} setGametime={setGametime} 
